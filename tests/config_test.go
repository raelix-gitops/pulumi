...
func TestConfigCommands(t *testing.T) {
	...
	t.Run("SanityTest", func(t *testing.T) {
		...
		assert.Equal(t,
			"error: configuration key 'outer' not found for stack 'organization/pulumi-test/test'",
			strings.Trim(stderr, "\r\n"))

		_, stderr = e.RunCommandExpectError("pulumi", "config", "get", "myList")
		assert.Equal(t,
			"error: configuration key 'myList' not found for stack 'organization/pulumi-test/test'",
			strings.Trim(stderr, "\r\n"))
